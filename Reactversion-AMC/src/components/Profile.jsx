...
const Profile = ({ currentUser, setCurrentUser, API_BASE_URL, showGlobalMessage }) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (typeof setCurrentUser === 'function') {
          setCurrentUser(updatedUser);  // ✅ This updates the global state
        }
  ...
  return (
    <div className="container mx-auto py-10 px-4 ">
      <div className="max-w-3xl mx-auto bg-white dark:bg-black text-white rounded-lg shadow-lg p-6">
        {/* Avatar */}
        ...
        <div className="flex justify-center mb-6 border-b border-gray-200 dark:border-gray-700">
          {['info', 'email', 'username', 'password'].map(tab => (
            <button
              ...
              className={`px-4 py-2 font-semibold text-lg transition-all duration-200 ${activeTab === tab
              ...
              role="tab"
            >
              {tab === 'info' ? 'Profile Info' : tab === 'email' ? 'Update Email' : tab === 'username' ? 'Update Username' : 'Change Password'}
            </button>
          ))}
        </div>
